...
class MtOrder
{
	...
	public static function dealOrder($data,$dpid,$type){
		...
		$income = $poiReceiveDetail->wmPoiReceiveCent/100;
		$orderArr['order_info'] = array('creat_at'=>date('Y-m-d H:i:s',$orderTime),'account_no'=>$obj->orderId,'classes'=>0,'username'=>'','site_id'=>0,'is_temp'=>1,'number'=>1,'order_status'=>$obj->status,'order_type'=>7,'should_total'=>$income,'reality_total'=>$obj->originalPrice,'takeout_typeid'=>0,'callno'=>$obj->daySeq,'paytype'=>$payType,'appointment_time'=>date('Y-m-d H:i:s',$deliveryTime),'remark'=>$obj->caution);
		...
		foreach ($array_detail as $key => $value) {
			$pTotal = $value['price'];
			$pAmount = $value['quantity'];
			$productTotal +=$pTotal*$pAmount;
			if(!empty($value['box_price'])){
				$boxNum = $value['box_num'];
				$productTotal +=$value['box_price']*$boxNum;
			}
		}
		...
		foreach ($array_detail as $key => $value) {
			...
			$pPrice = Helper::dealProductPrice($price, $productTotal, $income);
			...
			if(!$res){
				...
				$orderProduct = array('is_set'=>0,'set_id'=>0,'product_id'=>0,'product_name'=>$foodName.'(未对应菜品)','original_price'=>$price,'price'=>$pPrice,'amount'=>$amount,'zhiamount'=>1,'product_taste'=>array(),'product_promotion'=>array());
				array_push($orderArr['order_product'], $orderProduct);
				
				if(!empty($value['box_price'])){
					$pPrice = Helper::dealProductPrice($value['box_price'], $productTotal, $income);
					$orderProduct = array('is_set'=>0,'set_id'=>0,'product_id'=>0,'product_name'=>'餐盒费','original_price'=>$value['box_price'],'price'=>$pPrice,'amount'=>$value['box_num'],'zhiamount'=>1,'product_type'=>2,'product_taste'=>array(),'product_promotion'=>array());
					array_push($orderArr['order_product'], $orderProduct);
				}
			}else{
				if( $res['is_set']==0){
					...
					$orderProduct = array('is_set'=>$res['is_set'],'set_id'=>0,'product_id'=>$res['lid'],'product_name'=>$res['name'],'original_price'=>$price,'price'=>$pPrice,'amount'=>$amount,'zhiamount'=>$amount,'product_taste'=>$tasteArr,'product_promotion'=>array());
					array_push($orderArr['order_product'], $orderProduct);
				}else{
				...
				if(!empty($value['box_price'])){
					$pPrice = Helper::dealProductPrice($value['box_price'], $productTotal, $income);
					$orderProduct = array('is_set'=>'0','set_id'=>'0','product_id'=>'0','product_name'=>'餐盒费','original_price'=>$value['box_price'],'price'=>$pPrice,'amount'=>$value['box_num'],'zhiamount'=>$value['box_num'],'product_type'=>2,'product_taste'=>array(),'product_promotion'=>array());
					array_push($orderArr['order_product'], $orderProduct);
				}
			}
		}
