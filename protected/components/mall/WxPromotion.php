...
class WxPromotion
{
	...
	public function getUserPromotion(){
		...
		$sql = 'select t.private_promotion_id,t1.promotion_title,t1.promotion_type,t1.order_num from nb_private_branduser t,nb_private_promotion t1 where t.private_promotion_id=t1.lid and t.dpid=t1.dpid and t.brand_user_lid=:userId and t.dpid=:dpid and t.begin_time <= :now and :now <= end_time and t.is_used=1 and t.to_group=3 and t.delete_flag=0' .
			   ' union select t.private_promotion_id,t1.promotion_title,t1.promotion_type,t1.order_num from nb_private_branduser t,nb_private_promotion t1 where t.private_promotion_id=t1.lid and t.dpid=t1.dpid and t.brand_user_lid=:userId and t.dpid=:dpid and t.begin_time <= :now and :now <= end_time and t.is_used=1 and t.to_group=3 and t.delete_flag=0';
		$results = Yii::app()->db->createCommand($sql)->bindValue(':dpid',$this->dpid)->bindValue(':userId',$this->user['user_level_lid'])->bindValue(':now',$now)->queryAll();
