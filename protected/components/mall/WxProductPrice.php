	...
	public function price(){
		...
		if(!empty($this->set)){
			...
			if(!$this->set['is_normal_promotion']){
				//查出排他活动
				$sql = 'select t.product_id,t.is_set,t.is_discount,t.promotion_money,t.promotion_discount,t.order_num from nb_normal_promotion_detail t,nb_normal_promotion t1 where t.normal_promotion_id=t1.lid and t.dpid=t1.dpid and t.product_id=:productId and t.dpid=:dpid and t.is_set=0 and t.delete_flag=0 and t1.delete_flag=0 and t1.is_available=0 and t1.begin_time <= :now and t1.end_time >=:now and t1.promotion_type=0 order by t1.create_at desc';
				...
				if(!empty($promotion)){
					if($promotion['is_discount']){
					   ...
					   if($this->price > $this->product['original_price']*$promotion['promotion_discount']/100){
					   	 $this->price = number_format($this->product['original_price']*$promotion['promotion_discount']/100,2);
					   	 $this->promotion['is_discount'] = $promotion['is_discount'];
					   }
					}
				...
				foreach($allNarmanPromotions as $allPromotion){
					if($promotion['is_discount']){
					   ...
					   //打折
					   $this->price = number_format($this->price*$allPromotion['promotion_discount']/100,2);
					}
					$this->price = $this->price >0 ? $this->price : 0;
					$this->promotion['price'] = $this->price;
					if(!$this->price){
