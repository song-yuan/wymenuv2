...
class Server {
   ...
   /**
	...
	public function scene() {
		...
		$this->text('欢迎关注我要点单官方微信！'.$sql);exit;
	...
	public function sceneScanLog() {
		if($this->sceneId) {
			$time = time();
			$sql = 'INSERT INTO nb_scene_scan_log(scene_id, dpid, user_id, create_time, update_time) VALUES('.$this->sceneId.', '.$this->brandId.', '.$this->userId.', '.$time.', '.$time.')';
    ...
    public function cancelUnsubscribe() {
    	$sql = 'update nb_brand_user set unsubscribe = 0 where openid = "' . $this->postArr['FromUserName'] .'"';
        Yii::app()->db->createCommand($sql)->execute();
    }
