...
class Helper
{
	...
	static public function printKitchenAll(Order $order,Site $site,  SiteNo $siteNo , $reprint){		
		...
		//var_dump($site->floor_id,$orderProduct->product->printer_way_id);exit;
                ...
                $orderProducts = OrderProduct::model()->with('product')->findAll('t.order_id=:id and t.dpid=:dpid and is_print=0 and t.delete_flag=0' , array(':id'=>$order->lid,':dpid'=>$order->dpid));
