...
//                }
               ...
               // }//添加
                ...
                $payname="查询时间段";
                $printlen=(strlen($payname) + mb_strlen($payname,'UTF8')) / 2;
                array_push($listData,"01".$payname.str_pad("", 30-$printlen," ").$begin_time." 至 ".$end_time);
                array_push($listData,"br");
                array_push($listData,"00".str_pad('',48,'-'));
                $payname="客流";
                $printlen=(strlen($payname) + mb_strlen($payname,'UTF8')) / 2;//return array('status'=>false,'msg'=>$modeldata['all_number']);
                array_push($listData,"01".$payname.str_pad("", 30-$printlen," ").$modeldata['all_number']);
                array_push($listData,"br");
                $payname="单数";
                //return array('status'=>false,'msg'=>"123");
                $printlen=(strlen($payname) + mb_strlen($payname,'UTF8')) / 2;
                array_push($listData,"01".$payname.str_pad("", 30-$printlen," ").$modeldata['all_account']);
                array_push($listData,"br");
                $payname="销售额";
                $printlen=(strlen($payname) + mb_strlen($payname,'UTF8')) / 2;
                array_push($listData,"01".$payname.str_pad("", 30-$printlen," ").sprintf("%.2f",$moneydata['all_originalprice']));
                array_push($listData,"br");
                $payname="实收";
                $printlen=(strlen($payname) + mb_strlen($payname,'UTF8')) / 2;
                array_push($listData,"01".$payname.str_pad("", 30-$printlen," ").sprintf("%.2f",$modeldata['all_realprice']));
                array_push($listData,"br");
                $payname="优惠";
                $printlen=(strlen($payname) + mb_strlen($payname,'UTF8')) / 2;
                array_push($listData,"01".$payname.str_pad("", 30-$printlen," ").sprintf("%.2f",$moneydata['all_originalprice']-$modeldata['all_realprice']));
                array_push($listData,"br");
                $payname="人均";
                $printlen=(strlen($payname) + mb_strlen($payname,'UTF8')) / 2;
                array_push($listData,"01".$payname.str_pad("", 30-$printlen," ").sprintf("%.2f",$modeldata['all_realprice']/$modeldata['all_number']));
                array_push($listData,"br");
                $payname="单均";
                $printlen=(strlen($payname) + mb_strlen($payname,'UTF8')) / 2;
                array_push($listData,"01".$payname.str_pad("", 30-$printlen," ").sprintf("%.2f",$modeldata['all_realprice']/$modeldata['all_account']));
                array_push($listData,"br");
                array_push($listData,"00".str_pad('',48,'-'));
                array_push($listData,"00".Yii::app()->user->name."    ".date('Y-m-d H:i:s',time()));
                array_push($listData,"br");
                ...
                $payname="查询时间段";
                $printlen=(strlen($payname) + mb_strlen($payname,'UTF8')) / 2;
                array_push($listData,"00".$payname.str_pad("", 30-$printlen," ").$begin_time." 至 ".$end_time);
                array_push($listData,"br");
                array_push($listData,"00".str_pad('',48,'-'));
                $payname="产品类型";
                $nummoney="数量/金额";
                ...
                foreach ($incomes as $model)
                {
                	...
                	$printlen=(strlen($payname) + mb_strlen($payname,'UTF8')) / 2;
                	array_push($listData,"00".$payname.$model['all_num']."/".sprintf("%.2f",$model['all_price']).str_pad("", 30," "));
                	array_push($listData,"br");
//                 	$payname="数量/金额";
//                 	$printlen=(strlen($payname) + mb_strlen($payname,'UTF8')) / 2;
...
//                                    array_push($listData,"00".str_pad('',48,'-'));
                                    ...
                                    foreach($values as $value)
                                    {
                                        ...
                                        array_push($listData,"01".$productnum.".".$orderProduct->product->product_name.str_pad("",32-$printlen," ").str_pad($orderProduct->amount,4," ").$orderProduct->product->product_unit);	
                                        array_push($listData,"br");
                                        $orderProductTastes = OrderTaste::model()->with('taste')->findAll('t.order_id=:orderid and t.dpid=:dpid and t.is_order=0',  array(':orderid'=>$orderProduct->lid,':dpid'=>$orderProduct->dpid));
                                        $orderProductTasteEx = $orderProduct->taste_memo;                
