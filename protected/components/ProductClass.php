...
class ProductClass
{
	...
	public static function getHotsProduct($dpid = 0,$type = 1){
		$command = Yii::app()->db;
		switch($type){
			case 1:
			$sql = 'select t8.*,t9.order_id from (select t.price_discount,t.is_discount,t1.lid,t1.product_name, t1.original_price, t1.main_picture, t1.rank, t1.order_number, t1.favourite_number,0 as type from nb_product_discount t,nb_product t1 where t.product_id=t1.lid and t.dpid=:companyId and t1.status=0 and t1.delete_flag=0 and t1.is_show = 1 and t.is_set=0 and t.begin_time<:time and t.end_time>:time ' .
				   ...
				   ' LEFT JOIN (select order_id,product_id from nb_order_product t10 LEFT JOIN nb_order t11 on t10.order_id=t11.lid where t10.delete_flag=0 and t10.product_order_status=0 )t9 on t8.lid=t9.product_id ';
				   break;
			case 2:
			$sql = 'select t.*,t1.order_id from (select  lid,set_name as product_name, 0 as original_price, main_picture, rank, order_number, favourite_number, 1 as type from nb_product_set where dpid=:companyId and delete_flag=0)t' .
					' LEFT JOIN (select order_id,product_id from nb_order_product t2 LEFT JOIN nb_order t3 on t2.order_id=t3.lid where t2.delete_flag=0 and t2.product_order_status=0 )t1 on t.lid=t1.product_id';
				   break;
			case 3:
			$sql = 'select t.*,t1.order_id from ' .
			   	   ...
			   	   ' LEFT JOIN (select order_id,product_id from nb_order_product t2 LEFT JOIN nb_order t3 on t2.order_id=t3.lid where t2.delete_flag=0 and t2.product_order_status=0 )t1 on t.lid=t1.product_id order by favourite_number desc limit 10';
				   break;
			case 4:
			$sql = 'select t.*,t1.order_id from ' .
			   	   ' (select lid,product_name, original_price, main_picture, rank, order_number, favourite_number,0 as type from nb_product where dpid=:companyId and status=0 and delete_flag=0 and is_show = 1 union select  lid,set_name as product_name, 0 as original_price, main_picture, rank, order_number, favourite_number, 1 as type from nb_product_set where dpid=:companyId and delete_flag=0)t ' .
			   	   ' ' .
			   	   '' .
			   	   'LEFT JOIN (select order_id,product_id from nb_order_product t2 LEFT JOIN nb_order t3 on t2.order_id=t3.lid where t2.delete_flag=0 and t2.product_order_status=0 )t1 on t.lid=t1.product_id order by order_number desc limit 10';
				   break;
		}
		$conn = $command->createCommand($sql);
