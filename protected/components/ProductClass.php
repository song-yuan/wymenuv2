...
class ProductClass
{
	...
	public static function getCategoryProducts($dpid=0,$siteNoId=0)
	{
		...
		$sql = 'select t.*,t1.order_id from ' .
			   ...
			   'LEFT JOIN (select order_id,product_id from nb_order_product t2 LEFT JOIN nb_order t3 on t2.order_id=t3.lid where t3.site_id=:siteId and t2.delete_flag=0 )t1 on t.lid=t1.product_id';
