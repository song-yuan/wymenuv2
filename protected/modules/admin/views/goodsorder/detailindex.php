...
<div class="page-content">
    ...
    <!-- BEGIN PAGE HEADER-->
    <?php $this->widget('application.modules.admin.components.widgets.PageHeader', array('breadcrumbs'=>array(array('word'=>yii::t('app','供应链'),'url'=>$this->createUrl('comgoodsorder/list' , array('companyId'=>$this->companyId,'type'=>0))),array('word'=>yii::t('app','销售订单列表'),'url'=>$this->createUrl('goodsorder/index' , array('companyId'=>$this->companyId))),array('word'=>yii::t('app','销售订单明细'),'url'=>'')),'back'=>array('word'=>yii::t('app','返回'),'url'=>$this->createUrl('goodsorder/index' , array('companyId' => $this->companyId,'page'=>$papage)))));?>
        ...
        <div class="col-md-12">
            ...
            <div class="portlet box purple">
                <div class="portlet-title">
                    <div class="caption"><i class="fa fa-globe"></i><?php echo $name ;?> => <?php echo yii::t('app','销售定单明细列表');?></div>
                    <div class="actions">
                ...
                <?php if($model):?>
                    ...
                    <div style="vertical-align:middle;text-align: center;" <?php echo $plid;?>>
                        ...
                        <div class="actions">
                            ...
                            <span>&nbsp;</span>
                            <?php
                                if($model['order_status']==7 or 4){
                                    if($model['pass_reason']==''){
                                        echo "<span>审核人姓名：</span>".Yii::app()->user->name;
                                    }else{
                                        echo "<span>审核人姓名：</span>".$model['pass_reason'];
                                    }
                                }
                            ?>
                        </div>
                ...
                <div class="portlet-body" id="table-manage">
                    <table class="table table-striped table-bordered table-hover" id="sample_1">
                        ...
                        <tr style="background: lightblue;">
                            ...
                            <?php if($status<=3):?>
                                <th><?php echo yii::t('app','调整仓库');?>
                                    <input id="change_stock" class="btn yellow" type="button" value="确定" />&nbsp;
                                </th>
                            <?php endif;?>
                        </tr>
                        </thead>
                        ...
                        <?php foreach ($models as $model):?>
                            <tr class="odd gradeX" goods-id="<?php echo $model['goods_id'] ;?>">
                                ...
                                <td id="stock"><?php if ($model['stock_all'] == ''){ echo 0; }else{ echo $model['stock_all'];}?> </td>
                                <?php if($status<=3 ):?>
                                    <td >
                                        <select id="paymentid" name="paymentid" class="col-md-9 form-control stockselect" >
                                            <?php if($stocks):?>
                                                <?php foreach ($stocks as $stock):?>
                                                    <option lid="<?php echo $model['lid'];?>" olddpid="<?php echo $model['stock_dpid'];?>" value="<?php echo $stock['dpid'];?>" <?php if ($model['stock_dpid'] == $stock['dpid']){ echo 'selected';  }?> > <?php echo $stock['company_name'];?></option>
                                                <?php endforeach;?>
                                            <?php endif;?>
                                        </select>
                                    </td>
                        ...
                        <tr class="none" id="back_reason">
                            <td colspan="20" style="text-align: right;" class="form-group">
                                <input id="goods_back_reason" type="text" class="form-control" placeholder="请输审核人姓名和入驳回原因" />
                            </td>
                        </tr>
                        <tr class="none" id="pass_reason">
                            <td colspan="20" style="text-align: right" class="form_group">
                                <input id="goods_pass_reason" type="text" class="form-control" placeholder="请输入审核人姓名" />
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td colspan="20" style="text-align: right;">
                                ...
                                <input id="new_goods_delivery" type="button" class="btn blue" value="生成配货单" />&nbsp;
                            </td>
                                <?php endif;?>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $('#goods_pass').on('click',function(){
                var attr = $('#pass_reason').attr('class');
                if(attr == 'none'){
                    $('#pass_reason').removeAttr('class');
                }else {
                    var pass_reason = $('#goods_pass_reason').val();
                    var account_no = $(this).attr('gac');
                    if(confirm('确认通过店铺订单审核?')){
                        $.ajax({
                            url:'<?php echo $this->createUrl('goodsorder/orderCheck1',array('companyId'=>$this->companyId));?>',
                            data:{
                                order_status:7,
                                account_no:account_no,
            ...
            $('#paymentid').change(function(){
                var obj = $('#paymentid').find("option:selected");
                var pid = obj.val();
                var goods_id = $('#paymentid').parents('tr').attr('goods-id');
                //alert(pid);alert(goods_id);
                $.ajax({
                    url:'<?php echo $this->createUrl('goodsOrder/stocks',array('companyId'=>$this->companyId));?>',
                    data:{pid:pid,goods_id:goods_id},
                    success:function(data){
                        $('#stock').html(data);
                    }
                })
