...
<div class="page-content">
	...
	<div id="main2" name="main2" style="min-width: 500px;min-height:300px;display:none;background: white;" onMouseOver="this.style.backgroundColor='rgba(255,222,212,1)'" onmouseout="this.style.backgroundColor='white'">
		<div id="content"></div>
	</div>
	...
	<!-- BEGIN PAGE HEADER-->
	<?php $this->widget('application.modules.admin.components.widgets.PageHeader', array('back'=>array('word'=>yii::t('app','返回'),'url'=>$this->createUrl('comtopay/index' , array('companyId' => $this->companyId,)))));?>
	<!-- END PAGE HEADER-->
	<!-- BEGIN PAGE CONTENT-->
	<div class="row">
...
<script>
	...
	function genQrcode(that){
		...
		$.get('<?php echo $this->createUrl('/admin/comtopay/genWxQrcode');?>/dpid/'+id,function(data){
			if(data.status){
	...
	$('#serch').on('click',function(){
		...
		//alert(111);
         location.href="<?php echo $this->createUrl('comtopay/listchidren' , array('companyId'=>$this->companyId));?>/content/"+content;
	});
	...
	$('#area').change(function(){
	 ...
	 function changeselect(){
		 var province = $('#province').children('option:selected').val();
			...
			location.href="<?php echo $this->createUrl('comtopay/index' , array('companyId'=>$this->companyId));?>/province/"+province+"/city/"+city+"/area/"+area;
	...
	$('.setAppid').on('click',function(){
		...
		$('#appid_store').on('click',function(){
			...
			var url = "<?php echo $this->createUrl('comtopay/store',array('companyId'=>$this->companyId));?>/companyId/"+dpid+"/appid/"+appid+"/code/"+code+"/paytype/"+paytype+"/paychannel/"+paychannel;
	        $.ajax({
	            url:url,
	            type:'GET',
	            //data:orderid,//CF
