...
<div class="form-body">
	...
	<div class="form-group">
		<?php echo $form->label($model, 'recharge_cashcard',array('class' => 'col-md-3 control-label'));?>
		<div class="col-md-4">
			<select class="form-control select2 select2me" name="WeixinRecharge[recharge_cashcard][]" multiple>
				<?php foreach ($cashcards as $cashcard):?>
				<option value="<?php echo $cashcard['lid'];?>" <?php if(in_array($cashcard['lid'], $recashcards)){echo 'selected="selected"';}?>><?php echo $cashcard['cupon_title'].'('.$cashcard['sole_code'].')';?></option>
				<?php endforeach;?>
			</select>
		</div>
	</div>
