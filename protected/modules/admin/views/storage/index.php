...
<div class="page-content">
    ...
    <!-- BEGIN PAGE HEADER-->
    <?php $this->widget('application.modules.admin.components.widgets.PageHeader', array('breadcrumbs'=>array(array('word'=>yii::t('app','供应链'),'url'=>$this->createUrl('comgoodsorder/list' , array('companyId'=>$this->companyId))),array('word'=>yii::t('app','仓库列表'),'url'=>'')),'back'=>array('word'=>yii::t('app','返回'),'url'=>$this->createUrl('comgoodsorder/list' , array('companyId' => $this->companyId,)))));?>
    <!-- END PAGE HEADER-->
    <!-- BEGIN PAGE CONTENT-->
    <div class="row">
        <?php $form=$this->beginWidget('CActiveForm', array(
            'id' => 'storage-form',
            'action' => $this->createUrl('storage/delete', array('companyId' => $this->companyId)),
        ...
        <div class="col-md-12">
            ...
            <div class="portlet box purple">
                <div class="portlet-title">
                    <div class="caption"><i class="fa fa-globe"></i><?php echo yii::t('app','仓库列表');?></div>
                    <div class="actions">
                        <div class="btn-group">
                        ...
                        <div class="btn-group">
                            <input type="text" class="form-control" name="ccontent" id="ccontent" placeholder='手机号, 联系人, 仓库名' >
                        </div>
                        <div class="btn-group">
                        ...
                        <?php if(Yii::app()->params->master_slave=='m') : ?>
                            <?php if(Yii::app()->user->role == User::POWER_ADMIN||Yii::app()->user->role <= '5'):?>
                                <a href="<?php echo $this->createUrl('Storage/create', array('companyId' => $this->companyId));?>" class="btn blue"><i class="fa fa-pencil"></i> <?php echo yii::t('app','添加');?></a>
                                <div class="btn-group">
                            ...
                            <table class="table table-striped table-bordered table-hover" id="sample_1">
                                ...
                                <?php if($models) :?>
                                    <?php foreach ($models as $model):?>
                                        <tr class="odd gradeX">
                                            <td><?php if(Yii::app()->user->role >= User::POWER_ADMIN_VICE && Yii::app()->user->role <= User::ADMIN_AREA&&$model->type=="0"):?><?php else:?>
                                               <input type="checkbox" class="checkboxes" value="<?php echo $model->dpid;?>" name="companyIds[]" /><?php endif;?>
                                            </td>
                                            <?php if(Yii::app()->user->role < '5'): ?><td><?php echo $model->dpid;?></td><?php endif; ?>
                                            <td><a href="<?php if(Yii::app()->user->role != '4') echo $this->createUrl('storage/update',array('dpid' => $model->dpid,'companyId' => $this->companyId));?>" ><?php echo $model->company_name;?></a></td>
                                            ...
                                            <td class="center">
                                                <div class="actions">
                                                    ...
                                                    <a  class='btn green' style="margin-top: 5px;" href="<?php echo $this->createUrl('Storage/update',array('dpid' => $model->dpid,'companyId' => $this->companyId,'type' => $model->type));?>"><?php echo yii::t('app','编辑');?></a>
                                                    <?php endif; ?>
                                                    <a  class='btn green' style="margin-top: 5px;"  href="<?php echo $this->createUrl('tmall/list' , array('companyId' => $model->dpid));?>"><?php echo yii::t('app','选择');?></a>
    ...
    <script>
        ...
        $('#serch').on('click',function(){
            ...
            //alert(111);
            location.href="<?php echo $this->createUrl('Storage/index' , array('companyId'=>$this->companyId));?>/content/"+content;
        });
        ...
        function changeselect(){
            ...
            location.href="<?php echo $this->createUrl('Storage/index' , array('companyId'=>$this->companyId));?>/province/"+province+"/city/"+city+"/area/"+area;
        ...
        $('.setAppid').on('click',function(){
            ...
            var content = '<div style="width: 88%;margin-left: 6%;padding-top: 10px;"><input id="paytype" placeholder="1表示总部，2表示个人，0表示不开通。"/></div>'
                    ...
                    + '<div style="display: none;width: 88%;margin-left: 6%;padding-top: 10px;"><input id="dpid" placeholder="dpid" value="'+dpid+'"/></div>'
