...
<div class="page-content">
	...
	<!-- BEGIN PAGE HEADER-->
	<?php $this->widget('application.modules.admin.components.widgets.PageHeader', array('breadcrumbs'=>array(array('word'=>yii::t('app','进销存'),'url'=>$this->createUrl('comgoodsorder/list' , array('companyId'=>$this->companyId,'type'=>0))),array('word'=>yii::t('app','总部原料价格体系对应店铺'),'url'=>'')),'back'=>array('word'=>yii::t('app','返回'),'url'=>$this->createUrl('pricegroupM/index' , array('companyId' => $this->companyId,)))));?>
	...
	<?php $form=$this->beginWidget('CActiveForm', array(
		...
		'action' => $this->createUrl('depotGroup/index' , array('companyId' => $this->companyId)),
    ...
    <div class="tabbable tabbable-custom">
			...
			<div class="portlet box purple">
				<div class="portlet-title">
					<div class="caption"><i class="fa fa-globe"></i><?php echo yii::t('app','总部原料价格体系对应店铺');?></div>
					<div class="actions">
						<div class="btn-group">
					...
					<table class="table table-striped table-bordered table-hover" id="sample_1">
						...
						<?php foreach ($models as $model):?>
							<tr class="odd gradeX">
								...
								<td>
									<select name="<?php echo $model['dpid']; ?>" id="aa<?php echo $model['dpid']; ?>" class="btn select" style="border:1px solid gray;padding:2px 3px;">
										<?php if (!$groups):?>
											<option value="">亲 , 您还没有添加价格分组,默认总部价格</option>
										<?php else:?>
											<option value="0" <?php if ($model['material_price_group_id']==0) {echo 'selected';} ?>>-默认(总部)-</option>
											<?php foreach($groups as $group ): ?>
												<option value="<?php echo $group['lid']; ?>" <?php if ($group['lid']==$model['material_price_group_id']) {echo 'selected';} ?>>-<?php echo $group['group_name']; ?>-</option>
											<?php endforeach; ?>
										<?php endif;?>
									</select>
								</td>
	...
	<script type="text/javascript">
		...
		$('.select').change(function(){
			$(this).parent().parent().children().children('.checker').children('span').children('.checkboxes').attr('checked', 'checked');
			$(this).parent().parent().children().children('.checker').children('span').attr('class', 'checked');
		});
		function changeselect(){
			var province = $('#province').children('option:selected').val();
			var city = $('#city').children('option:selected').val();
			var area = $('#area').children('option:selected').val();
			location.href="<?php echo $this->createUrl('depotGroup/index' , array('companyId'=>$this->companyId));?>/province/"+province+"/city/"+city+"/area/"+area;
		}
		$('.saved').on('click',function(){
			...
			$.ajax({
	            type:'get',
				url:"<?php echo $this->createUrl('depotGroup/store',array('companyId'=>$this->companyId,));?>/arr/"+arrs,
				async: false,
	            cache:false,
	            dataType:'json',
		...
		$('#search').click(function(event) {
			/* Act on the event */
			var cname = $('#cname').val();
			location.href="<?php echo $this->createUrl('depotGroup/index' , array('companyId'=>$this->companyId));?>/cname/"+cname;
		});
