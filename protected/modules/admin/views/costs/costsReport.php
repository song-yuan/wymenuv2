...
<div class="page-content">
    ...
    <!-- BEGIN PAGE HEADER-->
    <?php $this->widget('application.modules.admin.components.widgets.PageHeader', array('head'=>yii::t('app','数据中心'),'subhead'=>yii::t('app','营业数据'),'breadcrumbs'=>array(array('word'=>yii::t('app','营业数据'),'url'=>$this->createUrl('statements/list' , array('companyId'=>$this->companyId,'type'=>0,))),array('word'=>yii::t('app','成本管控报表'),'url'=>'')),'back'=>array('word'=>yii::t('app','返回'),'url'=>$this->createUrl('statements/list' , array('companyId' => $this->companyId,'type'=>0)))));?>
    ...
    <div class="col-md-12">
        ...
        <div class="portlet box purple">
            <div class="portlet-title">
                <div class="caption"><i class="fa fa-globe"></i><?php echo yii::t('app','成本管控报表');?></div>
            	<div class="actions">
			...
			<table class="table table-striped table-bordered table-hover" id="sample_1">
		        ...
		        <tr class="odd gradeX">
					...
					<a class="btn blue" href="<?php if($cost_type==0){
										echo $this->createUrl('costs/update',array('companyId'=>$this->companyId,'lid'=>$value['lid'],'time'=>$time,'cost_type'=>$cost_type));
									}elseif($cost_type==1){
										echo $this->createUrl('costs/update',array('companyId'=>$this->companyId,'lid'=>$value['lid'],'month'=>$month,'cost_type'=>$cost_type));
									}elseif($cost_type==2){
										echo $this->createUrl('costs/update',array('companyId'=>$this->companyId,'lid'=>$value['lid'],'year'=>$year,'cost_type'=>$cost_type));
									}  ?>">编辑</a> 
		    	...
		    	<?php if($model2):?>
				<?php foreach ($model2 as $val):?>
		        <tr class="odd gradeX">
					<td style="color:red;">
					<?php
						if($cost_type==0){
							echo $val['material_name'].'<span style="font-size:12px;color:darkblue;">(批次-'.$val['logid'].')</span>';
						}else{
							echo $val['material_name'];
						}
					?>
					</td>

					<td><?php echo '原料消耗成本'; ?></td>
					<td><?php echo $time; ?></td>
					<td>
					<?php
						if($cost_type==0){
					...
					<td><?php echo sprintf("%.2f", $val['unit_price']*$val['num']); ?></td>
					<td><?php echo sprintf("%.2f",$val['unit_price']*$val['num']); $pay += $val['unit_price']*$val['num'];?></td>
				...
				<?php
					...
					if($model3):
				?>
				<?php foreach ($model3 as $income):?>
		        <tr class="odd gradeX">
					<td style="color:green;">营业收入</td>
					<td>营业收入</td>
					<td><?php
							if($cost_type==0){
								echo $income['y_all'].'-'.$income['m_all'].'-'.$income['d_all'];
							}elseif($cost_type==1){
								echo $income['y_all'].'-'.$income['m_all'];
							}elseif($cost_type==2){
								echo $income['y_all'];
							}
					  ?></td>
					<td><?php
							if($cost_type==0){
								echo '<span style="color:green;">日营业收入</span>';
							}elseif($cost_type==1){
								echo '<span style="color:green;">月营业收入</span>';
							}elseif($cost_type==2){
								echo '<span style="color:green;">年营业收入</span>';
							}
						?></td>
					<td><?php echo $income['all_realprice']; ?></td>
					<td><?php echo $income['all_realprice']; $in += $income['all_realprice'];?></td>
		        ...
		        <tr class="odd gradeX" >
		               ...
		               <th style="background: #ccc!important;"></th>
		               <th style="background: #ccc!important;"></th>
		               <th style="background: #ccc!important;"><?php echo yii::t('app','收益(收入[含退款])');?></th>
		               <th style="background: #ccc!important;"> + <?php echo sprintf("%.2f", $in);?>元</th>
		        </tr>
...
jQuery(document).ready(function(){
    ...
    $('#btn_time_query').click(function time() {
     	...
     	if (text==0) {
     		var time = $('#time').val();
     		location.href="<?php echo $this->createUrl('costs/costsReport' , array('companyId'=>$this->companyId ));?>/time/"+time+"/cost_type/"+text;
     	}else if(text==1){
     		var month = $('#month').val();
     		location.href="<?php echo $this->createUrl('costs/costsReport' , array('companyId'=>$this->companyId ));?>/month/"+month+"/cost_type/"+text;
     	}else if(text==2){
     		var year = $('#year').val();
     		location.href="<?php echo $this->createUrl('costs/costsReport' , array('companyId'=>$this->companyId ));?>/year/"+year+"/cost_type/"+text;
     	}
	});
    ...
    $('#create').click(function() {
     	...
     	var text = $('#text').val();
     	location.href="<?php echo $this->createUrl('costs/create' , array('companyId'=>$this->companyId ));?>/time/"+time+"/cost_type/"+text;
	});
	...
	$('#excel').click(function excel(){
		var cost_type = $('#text').val();
		if(confirm('确认导出并且下载Excel文件吗？')){
			if (cost_type==0) {
	     		var time = $('#time').val();
	     		location.href="<?php echo $this->createUrl('costs/costsReportExport' , array('companyId'=>$this->companyId ));?>/time/"+time+"/cost_type/"+cost_type;
	     	}else if(cost_type==1){
	     		var month = $('#month').val();
	     		location.href="<?php echo $this->createUrl('costs/costsReportExport' , array('companyId'=>$this->companyId ));?>/month/"+month+"/cost_type/"+cost_type;
	     	}else if(cost_type==2){
	     		var year = $('#year').val();
	     		location.href="<?php echo $this->createUrl('costs/costsReportExport' , array('companyId'=>$this->companyId ));?>/year/"+year+"/cost_type/"+cost_type;
	     	}
		}
	});
