        	...
        	foreach ($dpids as $dpid){
        		if(!empty($catep1)){
        			foreach($catep1 as $category){
        				$catep = ProductCategory::model()->find('chs_code=:ccode and dpid=:companyId' , array(':ccode'=>$category['chs_code'],':companyId'=>$dpid));
        				if($catep){
...
// 	                        $this->redirect(array('copyproduct/index' , 'companyId' => $this->companyId));
	                        ...
	                        $command = $db->createCommand()->insert('nb_product_category',$data);
	                        	...
	                        	//var_dump($model);exit;
 	                        	$self = ProductCategory::model()->find('lid=:pid and dpid=:dpid' , array(':pid'=>$id,':dpid'=>$dpid ));
 	                        	//var_dump($self);exit;
	                        	if($self->pid!='0'){
...
// 	                        	
        			...
        			if($catep2){
        				foreach ($catep2 as $category){
        					$sql = 'select t.chs_code,t.tree from nb_product_category t where t.lid='.$category['pid'].' and t.dpid='.$this->companyId;
        					$command = $db->createCommand($sql);
        					$sqltree = $command->queryRow();
        					$chscode = $sqltree['chs_code'];
        					//$chscodetree = $sqltree['tree'];
        					$catep = ProductCategory::model()->find('chs_code=:ccode and dpid=:companyId' , array(':ccode'=>$category['chs_code'],':companyId'=>$dpid));
        					$cateptree = ProductCategory::model()->find('chs_code=:ccode and dpid=:companyId' , array(':ccode'=>$chscode,':companyId'=>$dpid));
        		...
        		if($products){
        			foreach ($phscodes as $prodhscode){
        				$producto = Product::model()->find('phs_code=:pcode and dpid=:companyId' , array(':pcode'=>$prodhscode,':companyId'=>$dpid));
        				$product =  Product::model()->find('phs_code=:pcode and dpid=:companyId' , array(':pcode'=>$prodhscode,':companyId'=>$this->companyId));
        				$categoryId = ProductCategory::model()->find('chs_code=:ccode and dpid=:companyId' , array(':ccode'=>$product['chs_code'],':companyId'=>$dpid));
        				//var_dump($product,$producto,$categoryId);//exit;
        				//var_dump(!empty($producto));exit;
        				if((!empty($product))&&(empty($producto))){
        					//var_dump($catep);exit;
