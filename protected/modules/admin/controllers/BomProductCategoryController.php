...
class BomproductCategoryController extends BackendController
{
	...
	public function actionCreate() {
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if($category){
				$this->redirect(array('bomproductCategory/index' , 'id'=>$category->lid,'companyId' => $this->companyId));
			}			
			...
			if($model->save()){
				...
				$this->redirect(array('bomproductCategory/index' , 'id'=>$self->lid,'companyId' => $this->companyId));
			}else{
				Yii::app()->user->setFlash('error' ,yii::t('app', '添加失败'));
				$this->redirect(array('bomproductCategory/index' ,'companyId' => $this->companyId));
			}
		}
		$this->render('_form1' , array(
				'model' => $model,
				'action' => $this->createUrl('bomproductCategory/create' , array('companyId'=>$this->companyId))
		));
	}
	public function actionUpdate() {
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if($model->save()){
				...
				$this->redirect(array('bomproductCategory/index' , 'id'=>$model->lid,'companyId' => $this->companyId));
			}
		}
		$this->render('_form1' , array(
				'model' => $model,
				'action' => $this->createUrl('bomproductCategory/update' , array(
	...
	public function actionDelete(){
		...
		$this->redirect(array('bomproductCategory/index','companyId'=>$this->companyId,'id'=>$model->pid));
	}
