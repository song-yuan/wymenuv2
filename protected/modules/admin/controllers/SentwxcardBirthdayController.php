<?php
class SentwxcardBirthdayController extends BackendController
{
    ...
    public function actionIndex(){
    	...
    	if($is_new == '1'){
    		...
    		Yii::app()->db->createCommand('update nb_sentwxcard_promotion set is_available ="0",is_sync ='.$is_sync.' where type = "2" and dpid = :companyId')
    		->execute(array( ':companyId' => $this->companyId));
    		$this->redirect(array('sentwxcardBirthday/index' , 'companyId' => $this->companyId)) ;
    	}elseif ($is_new =='3'){
    		Yii::app()->db->createCommand('update nb_sentwxcard_promotion set is_available ="1",is_sync ='.$is_sync.' where type = "2" and dpid = :companyId')
    		->execute(array( ':companyId' => $this->companyId));
    		$this->redirect(array('sentwxcardBirthday/index' , 'companyId' => $this->companyId)) ;
    	}else{
