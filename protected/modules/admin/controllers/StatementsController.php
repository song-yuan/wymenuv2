	...
	*/
	...
	public function actionCeshiproductsetReport(){
		...
		if($text==1){
			...
			$sql = 'select c.* from( 
					select k.*,sum(k.zhiamount) as all_setnum,sum(k.zhiamount*k.all_price) as all_setprice,sum(k.zhiamount*k.all_oriprice) as all_orisetprice 
					from ( 
...
// 			);
	...
	public function actionCeshiproductReportExport(){
		...
		$sql = 'select k.lid from nb_order k where k.order_type = '.$ordertype.' and k.order_status in(3,4,8) and k.dpid = '.$this->companyId.' and k.create_at >="'.$begin_time.' 00:00:00" and k.create_at <="'.$end_time.' 23:59:59" group by k.user_id,k.account_no,k.create_at';
		$orders = Yii::app()->db->createCommand($sql)->queryAll();
		$ords ='0000000000';
		foreach ($orders as $order){
			$ords = $ords .','.$order['lid'];
