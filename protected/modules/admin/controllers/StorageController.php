...
class StorageController extends BackendController{
    ...
    public function actionIndex(){
        ...
        if(Yii::app()->user->role < '5'){
            if ($content=='') {
                $criteria->condition =' t.delete_flag=0 and t.type=2';
            }else{
                $criteria->condition ='t.type=2 and t.delete_flag=0';
            }
        }else if(Yii::app()->user->role >= '5' && Yii::app()->user->role <= '9') {
            //var_dump(Yii::app()->user->role);exit;
            $criteria->condition ='t.type=2 and t.delete_flag=0 and t.dpid in (select tt.dpid from nb_company tt where tt.comp_dpid='.Yii::app()->user->companyId.' and tt.delete_flag=0 ) or t.dpid='.Yii::app()->user->companyId;
        }else{
            $criteria->condition = 't.type=2 and t.delete_flag=0 and t.dpid='.Yii::app()->user->companyId ;
        }
        $province = $provinces;
        $city = $citys;
        $area = $areas;
        ...
        if ($content) {
            if (is_numeric($content)) {//is_numeric()检查变量是否是数字
            ...
            if(Yii::app()->user->role <= User::POWER_ADMIN_VICE){
                if(Yii::app()->request->isPostRequest) {
                    ...
                    if($model->save()){
                        ...
                        $this->redirect(array('Storage/index','companyId'=> $this->companyId));
                    } else {
