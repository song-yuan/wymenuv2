...
class CompanyGroupController extends BackendController
{
    ...
    public function actionPeisindex()
    {
    	$provinces = Yii::app()->request->getParam('province',0);
    	$citys = Yii::app()->request->getParam('city',0);
    	$areas = Yii::app()->request->getParam('area',0);
    	...
    	$sql2 = 'select * from nb_peisong_group where dpid = '.$dpid.' and delete_flag=0';
    	$groups = Yii::app()->db->createCommand($sql2)->queryALL();
    	...
    	if(Yii::app()->request->isPostRequest) {
    		...
    		Yii::app()->user->setFlash('success' ,yii::t('app', '修改成功'));
    		$this->redirect(array('companyGroup/peisindex' , 'companyId' => $dpid));
    	}
    	// p($models);
    	$this->render('peisindex',array(
    			'models'=>$models,
    			'groups'=>$groups,
    			'province'=>$provinces,
    			'city'=>$citys,
    ...
    public function actionPeistore()
    {
    	$pcode = Yii::app()->request->getParam('arr');
    	$arr = explode(':',$pcode);
    	//var_dump($arr);exit;
