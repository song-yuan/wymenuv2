...
<div class="dp">
	<form class="mui-input-group" method="POST" onsubmit="return validate()">
		<div class="mui-input-row">
			<label>盘损原因</label>
			<input id="name" type="text" name="Retreat[name]" placeholder="输入类型名称" value="<?php echo $model->name;?>">
		</div>
		<div class="mui-input-row" style="height:auto;">
			<label>说明</label>
			<textarea id="tip" name="Retreat[tip]" rows="5" placeholder="输入说明"><?php echo $model->tip;?></textarea>
		</div>
...
function back(){
	location.href = "<?php echo $this->createUrl('appReport/kcpsyy',array('companyId'=>$this->companyId));?>";
}
</script>