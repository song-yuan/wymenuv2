		...
		<div class="mui-content">
			<form class="mui-input-group margin-top" action="<?php echo $this->createUrl('address/editaddress',array('companyId'=>$this->companyId)) ?>" method="POST">
				<div class="mui-input-row ">
					...
					<input type="hidden" name="lid" value=" <?php echo $model['lid']; ?>">
					<input type="text" name="name" id="name" class="mui-input-clear" placeholder="请输入收货人姓名" value=" <?php echo $model['name']; ?>" >
				</div>
				<div class="mui-input-row">
					<label class="color-h-gray">联系电话</label>
					<input type="number" name="mobile" id="mobile" class="mui-input-clear mui-numbox-input" placeholder="请输入收货人联系电话" oninput="if(value.length>11)value=value.slice(0,11)" value="<?php echo $model['mobile']; ?>">
				</div>
				<ul class="mui-table-view mui-table-view-chevron mui-input-row">
					<li class="mui-table-view-cell">
						<a class="mui-navigate-right" id='showCityPicker3'>
							...
							<div id='cityResult3' class="left border-none padding-top1 fontsize"><?php echo $model['pcc']; ?></div>
							<input type="hidden" name="pcc" value=" <?php echo $model['pcc']; ?>" id="cityResult4" />
						</a>
					</li>
				</ul>
				<div class="mui-input-row" style="height:80px;">
					<textarea id="textarea" name="street" rows="3" placeholder="详细地址" ><?php echo $model['street']; ?></textarea>
				</div>
				<div class="mui-input-row mui-checkbox">
					<label class="color-h-gray aa" for="mo">设置为默认</label>
					<input name="default_address" value="1" id="mo" type="checkbox" <?php if ($model['default_address'] == 1) { echo 'checked';} ?> >
				</div>
