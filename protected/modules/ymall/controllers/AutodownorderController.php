...
class AutodownorderController extends BaseYmallController
{
			...
			foreach ($stocks as  $stock) {
				...
				if($stock['max_stock']){
					...
					if (($stock['max_stock']*2/3) > $stock['stock']  ) {
						...
						$sql1 = 'select g.goods_code,g.goods_name,g.lid as glid,g.main_picture,g.price,g.goods_unit,c.company_name,c.dpid,mc.lid,mc.category_name,gm.material_code,mu.unit_ratio from nb_goods g '
						...
						// p($product);
						if ($product) {
							$num = ceil(($stock['max_stock']-$stock['stock'])/$product['unit_ratio']);
							...
							$goods_cart->promotion_price = $product['price'];
							$goods_cart->price = $product['price'];
							$goods_cart->num = $num;
							$goods_cart->end_time = '';
	 ...
	 *            [description]  查询店铺原料 剩余库存
	 *            				 循环比较库存与预采购天数*日均消耗
	 *            				 对低于预采购天数*日均消耗的进行购物车添加(需要系数换算)
	 * 							 (日均消耗*预采购天数-现有库存)/单位系数---->补仓2
	 */
	public function actionIndex2()
	{
		...
		$user_id = substr(Yii::app()->user->userId,0,10);
		$user_name = Yii::app()->user->name;
		$day_nums = Yii::app()->request->getParam('day_nums',7);
		$db = Yii::app()->db;
		$info = GoodsCarts::model()->deleteAll('dpid=:dpid and  user_id=:user_id',array(':dpid'=>$this->companyId,':user_id'=>$user_id));
		$companyId = Company::model()->find('dpid=:dpid and delete_flag=0',array(':dpid'=>$this->companyId))->comp_dpid;
		if (!$companyId) {
		...
		$sqld='select k.max_stock,k.safe_stock,k.avg_sales,k.material_id FROM ( select a.* from nb_product_material_safe a where a.create_at>DATE_SUB(NOW(), INTERVAL 2 DAY) and a.dpid = '.$this->companyId
		.' order by a.create_at desc) k group by k.material_id';
		$stock_s = $db->createCommand($sqld)->queryAll();
		$stocks_arr=array();
		foreach ($stock_s as $key => $value) {
		...
		foreach ($stocks_goods as $key => $val) {
			...
			if (isset($stocks_arr['lid'.$val['lid']])) {
				$stocks['lid'.$val['lid']]['max_stock']=$stocks_arr['lid'.$val['lid']]['max_stock'];
				$stocks['lid'.$val['lid']]['safe_stock']=$stocks_arr['lid'.$val['lid']]['safe_stock'];
				$stocks['lid'.$val['lid']]['avg_sales']=$stocks_arr['lid'.$val['lid']]['avg_sales'];
			} else{
				$stocks['lid'.$val['lid']]['max_stock']='';
				$stocks['lid'.$val['lid']]['safe_stock']='';
				$stocks['lid'.$val['lid']]['avg_sales']='';
			}
		}
		// p($stocks);
		...
		if($stocks){
			// p($stocks);
			foreach ($stocks as  $stock) {
				//判断日均消耗乘以采购天数和现有库存的大小
				// 大:采购
				// 小:跳过
				if($stock['max_stock']){
					//如果原料库存少于最大库存的一半就将该产品列入添加队列
					if ($stock['avg_sales']*$day_nums > $stock['stock'] ) {
						//按照总部指定仓库查询原料 , 没有则为自采购原料
						$sql1 = 'select g.goods_code,g.goods_name,g.lid as glid,g.main_picture,g.price,g.goods_unit,c.company_name,c.dpid,mc.lid,mc.category_name,gm.material_code,mu.unit_ratio from nb_goods g '
								.' left join nb_company c on(c.dpid=g.dpid) '
								.' left join nb_material_category mc on (mc.lid=g.category_id )'
						...
						// p($product);
						if ($product) {
							$num = ceil(($stock['avg_sales']*$day_nums-$stock['stock'])/$product['unit_ratio']);
							$goods_cart = new GoodsCarts();
							$se=new Sequence("goods_carts");
							$lid = $se->nextval();
							$is_sync = DataSync::getInitSync();
