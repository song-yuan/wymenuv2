...
function showList(items){
	...
	for(var i in items){
		...
		if(leftHeight > rightHeight){
			...
			var trAddinfo = '';
			if(item.order_id)
				 trAddinfo +='<div class="clear"></div></div><div class="addCart view hasorder" style="float:left" product-id="'+item.lid+'" store="'+item.store_number+'" type="'+item.type+'" price="'+item.original_price+'"></div><div class="clear"></div> </div></div>';
			else
